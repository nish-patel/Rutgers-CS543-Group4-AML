...
const spiralCCTooltip = d => (true ? `#cf: ${d['count']}` : `#cf: ${d['count']} #sub-bucket: ${Object.keys(d['subBucket']).length}`) + ` minSize: V${d['minCC'][1]}E${d['minCC'][0]} aveSize: V${(d['vertices']/d['count']).toFixed(0)}E${(d['edges']/d['count']).toFixed(0)} maxSize: V${d['maxCC'][1]}E${d['maxCC'][0]}`
...
function drawMap(datas, buildingMapControls, divName) {
    ...
    let interval = 1;
    if (width < layers.length * 48) {
        interval = Math.ceil(layers.length * 24 / width) + 1;
    }
    console.log(layers.length, width)
    ...
    svg.append("g")
        .attr("class", 'axis')
        .attr("transform", `translate(0, ${height})`)
        .call(d3.axisBottom(x).tickValues(layers.filter((d, idx) => idx % interval === 0 || idx === layers.length - 1).map(d => brokenX[d])).tickFormat(d => revBrokenX[d]))
        .selectAll("text")
    ...
    // console.log(getBuilding2BucketPeel(bucketPeel2Building))

    return {building2BucketPeel: getBuilding2BucketPeel(bucketPeel2Building), glyphData: glyphData, buckSize: getBuckSize(buildingList, spiralList)};
};

function getBuckSize(buildingList, spiralList) {
    const retval = {}
    buildingList.forEach(d => {
        retval[`${d.layer}-${d.bucket}`] = [d['lccList'][0]['vertices'], d['lccList'][0]['edges']]
    })
    spiralList.forEach(d => {
        retval[`${d.layer}-${d.bucket}`] = [d['vertices'], d['edges']]
    })
    // console.log(retval);
    return retval;
}
